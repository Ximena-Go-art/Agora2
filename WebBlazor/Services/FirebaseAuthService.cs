...
namespace WebBlazor.Services
{
    public class FirebaseAuthService
    {
        ...
        public async Task<FirebaseUser?> SignInWithEmailPassword(string email, string password, bool rememberPasword)
        {
            var user = await _jsRuntime.InvokeAsync<FirebaseUser?>("firebaseAuth.signInWithEmailPassword", email, password, rememberPasword);
            if (user != null)
            {
                CurrentUser = user;
